#!/usr/bin/env node
/**
 * HTTP entrypoint for recipes-mcp. Mirrors shoes-mcp/orders-mcp http.ts —
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.error('[recipes-mcp] request error:', err);
    if (!res.headersSent) {
...
server.listen(PORT, () => {
  // eslint-disable-next-line no-console
  console.log(`[recipes-mcp] listening on http://localhost:${PORT}/mcp`);
});
